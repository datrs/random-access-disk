...
async fn can_del_long_middle() {
    ...
    file.write((MULTI_BLOCK_LEN + 5) as u64, b"to all the ")
        .await
        .unwrap();
    file.write((MULTI_BLOCK_LEN + 16) as u64, b"people")
        .await
        .unwrap();
    ...
    assert_eq!(zeros, vec![0; 5]);
    let to_all_the_people = file.read((MULTI_BLOCK_LEN + 5) as u64, 17).await.unwrap();
    assert_eq!(
...
async fn can_del_long_more_than_block() {
    ...
    assert_eq!(zeros, vec![0; 5]);
    file.del(5, (EXACT_TO_THIRD_BLOCK_LEN * 2) as u64)
        .await
        .unwrap();
